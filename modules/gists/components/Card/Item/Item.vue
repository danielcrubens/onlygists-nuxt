<template>
  <div class="card">
    <Card>
      ...
      <template #footer>
        <Button @click="handleClick" v-if="isFree" label="Baixar gratuitamente" class="w-full" icon="pi pi-shopping-bag"
          icon-pos="right" />
        <Button @click="handleClick" v-else :label="`Comprar por ${amount}`" class="w-full" icon="pi pi-shopping-bag"
          icon-pos="right" />
      </template>
    </Card>
  </div>
